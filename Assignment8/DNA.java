...
public class DNA {
    ...
    public void createDatabaseOfProfiles ( String filename ) {
        ...
        for(int i = 0;i < p;i++) {
            ...
            database[i] = new Profile(name , null, null, first, second);
        }
    ...
    Profile p = new Profile("Unknown", null,null, seq1, seq2);
    ...
    public STR findSTRInSequence ( String sequence, String STR ) {
        ...
        if(sequence.indexOf(STR) == -1 || STR.length() > sequence.length()) {
                STR a = new STR( STR, 0);
                return a;
            }
        ...
        while(y == false) {
            if (sequence.indexOf(STR) == 0) {
                ...
                sequence = sequence.substring(sequence.indexOf(STR) + STR.length()); 
            }
            else if (sequence.indexOf(STR) > 0) {
                sequence = sequence.substring(sequence.indexOf(STR) + STR.length());
                repeats = 0; 
                repeats += 1;
                if (repeats > max) {max = repeats;}
            }
            else if (sequence.indexOf(STR) == -1) {
                y = true;
            }
            

        }
        STR b = new STR(STR, max);
    ...
    public void createProfileSTRs ( Profile profile, String[] allSTRs ) {
        ...
        for (int i = 0; i < allSTRs.length;i++){
            S1_STRs[i] = findSTRInSequence(seq1, allSTRs[i]);
            S2_STRs[i] = findSTRInSequence(seq2, allSTRs[i]);
        }
    ...
    public void createDatabaseSTRs() {
        ...
        for(int i = 0;i < database.length; i++) {
            createProfileSTRs(database[i], STRsOfInterest ); //not sure if rght
        }
    ...
    public boolean identicalSTRs ( STR[] s1, STR[] s2 ) {
        ...
        for(int i = 0; i < s1.length; i ++) {
            if ((s1[i].getSTR().equals(s2[i].getSTR()) == false) || s1[i].getRepeats() != s2[i].getRepeats()) {
                return false;
            }
        ...
        for ( int p1 = 0; p1 < possibleParent1.size(); p1++ ) {
           ...
           for ( int p2 = 0; p2 < possibleParent2.size(); p2++ ) {
               ...
               if ( !possibleParent1.get(p1).equals(possibleParent2.get(p2)) ) {
                   ...
                   if ( punnetSquare(possibleParent2.get(p2).getS2_STRs(), S1_STRs, possibleParent1.get(p1).getS2_STRs(), S2_STRs)) {
